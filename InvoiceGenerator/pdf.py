...
class SimpleInvoice(BaseInvoice):
    ...
    def _drawTitle(self):
        ...
        if not self.invoice.use_tax:
            self.pdf.drawString(
                (self.LEFT + 90) * mm,
                self.TOP*mm,
                _(u'Bill No: %s') % self.invoice.number,
            )
        else:
            self.pdf.drawString(
                (self.LEFT + 90) * mm,
    ...
    def _drawAddress(self, top, left, width, height, header_string, address):
        ...
        default = ParagraphStyle('default', fontName='DejaVu', fontSize=9, leading=8.5)
    ...
    def _drawProvider(self, TOP, LEFT):
        self._drawAddress(TOP, LEFT, 88, 36, _(u'BlubenHolidays'), self.invoice.provider)
    ...
    def _drawPayment(self, TOP, LEFT):
        ...
        lines = [
            ...
            '%s: %s' % (_(u'Payment Type'), self.invoice.provider.bank_account_str()),
        ]
    ...
    def _drawCreator(self, TOP, LEFT):
        ...
        self.pdf.drawString((LEFT + 8) * mm, (TOP - 5) * mm - height, '%s. %s' % (_(u'This is a System Generated Invoice'), self.invoice.creator.name))
    ...
    def _drawDates(self, TOP, LEFT):
        ...
        if self.invoice.date and self.invoice.use_tax:
            ...
            items.append((LEFT * mm, '%s: %s' % (_(u'Date'), format_date(self.invoice.date, locale=lang))))
